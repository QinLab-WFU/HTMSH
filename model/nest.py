...
#         url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vt3p-weights/jx_nest_tiny-e3428fb9.pth'),
#
# Add full connection coding module
class HashLayer(nn.Module):

    def __init__(self, config, hash_bit):
        super(HashLayer, self).__init__()
        self.head = nn.Linear(512 * 14 * 14, 1024)
...
class Nest(nn.Module):
    ...
    def __init__(self, config, num_levels=3, embed_dims=(128, 256, 512),num_heads=(4, 8, 16), depths=(2, 2, 20),
        ...
        self.hashlayer_train = HashLayer(self.num_features*12, config["bit"])
        # self.MultiScale_feature_fusion_module = MultiScale_feature_fusion_module(embed_dims[-1])

        # Classifier
        self.global_pool, self.head_cls = create_classifier(config["bit"], self.num_classes, pool_type=global_pool)
    ...
    def forward_features(self, x):
        """ x shape (B, C, H, W)
        """
        # 输入224*224 分成4*4的块 224/4*224/4=56*56=3136块
